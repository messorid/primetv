          ...
          <AnimatePresence mode="wait">
            ...
            {step === 2 && (
              <motion.form key="step2" initial="hidden" animate="visible" exit="hidden" variants={variants} transition={{ duration: 0.5 }} className="space-y-5" onSubmit={(e) => { e.preventDefault(); setStep(3) }}>
                <h3 className="text-xl font-semibold">TV Details</h3>
                {tvDetails.map((tv, index) => (
                  <div key={index} className="border p-4 rounded-lg space-y-3 mb-4">
                    <select name="tvType" value={tv.tvType} onChange={(e) => handleTvChange(index, e)} className="w-full border rounded p-2" required>
                    ...
                    <select name="tvSize" value={tv.tvSize} onChange={(e) => handleTvChange(index, e)} className="w-full border rounded p-2" required>
                    ...
                    <select name="wallType" value={tv.wallType} onChange={(e) => handleTvChange(index, e)} className="w-full border rounded p-2" required>
                    ...
                    <select name="hideCables" value={tv.hideCables} onChange={(e) => handleTvChange(index, e)} className="w-full border rounded p-2" required>
                      <option value="">Hide Cables?</option>
                      <option value="Yes">Yes (+$60)</option>
                      <option value="No">No</option>
                    </select>
                    <textarea name="comments" value={tv.comments} onChange={(e) => handleTvChange(index, e)} placeholder="Additional Comments (optional)" className="w-full border rounded p-2 mt-1" rows="2" />
                  </div>
                ))}
            ...
            {step === 3 && (
              <motion.form key="step3" initial="hidden" animate="visible" exit="hidden" variants={variants} transition={{ duration: 0.5 }} className="space-y-4" onSubmit={handleSubmit}>
