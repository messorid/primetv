...
const stats = [
  ...
  { value: "4.8", label: "Google Rating", icon: <Star size={13} className="text-yellow-400 fill-yellow-400" /> },
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-6 lg:px-10 pt-10 pb-16 lg:pt-14 lg:pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <motion.div
            ...
            className="relative flex flex-col items-center lg:justify-end"
          >
            {/* Floating card: Google review.
                Desktop only — these are sized to overhang the image, and the
                two-column layout gives them somewhere to go. On mobile the
                image shrinks to the viewport and they end up covering most of
                the photo, so the same proof is rendered as a row underneath. */}
            <motion.div
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ delay: 0.8, duration: 0.5 }}
              className="hidden lg:flex absolute -left-6 top-8 z-20 bg-white rounded-2xl shadow-2xl border border-black/5 px-4 py-3 items-center gap-3 w-52"
            >
              ...
              <div>
                ...
                <p className="text-[11px] font-bold text-black mt-0.5">4.8 out of 5</p>
            ...
            <motion.div
              ...
              className="hidden lg:flex absolute -right-4 bottom-12 z-20 bg-black rounded-2xl shadow-2xl px-4 py-3 items-center gap-3 w-48"
            >
            ...
            {/* Same proof as the floating cards, but below the photo where it
                doesn't cover the work we're trying to show. */}
            <div className="lg:hidden mt-4 w-full max-w-md grid grid-cols-2 gap-2.5">
              <div className="flex items-center gap-2.5 rounded-2xl border border-black/8 bg-white px-3 py-2.5 shadow-sm">
                <div className="flex items-center justify-center size-8 rounded-lg bg-[#E50914] shrink-0">
                  <svg viewBox="0 0 24 24" className="size-4 fill-white"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
                </div>
                <div className="min-w-0">
                  <p className="text-[11px] font-bold text-black leading-tight">4.8 out of 5</p>
                  <p className="text-[10px] text-black/50 leading-tight">Google Reviews</p>
                </div>
              </div>

              <div className="flex items-center gap-2.5 rounded-2xl bg-black px-3 py-2.5 shadow-sm">
                <div className="flex items-center justify-center size-8 rounded-lg bg-emerald-500 shrink-0">
                  <svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" className="size-4">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
                </div>
                <div className="min-w-0">
                  <p className="text-[11px] font-bold text-white leading-tight">Job completed</p>
                  <p className="text-[10px] text-white/50 leading-tight truncate">Brentwood · Today</p>
                </div>
              </div>
            </div>
          </motion.div>
